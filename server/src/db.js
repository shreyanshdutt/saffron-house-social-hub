...
const REQUIRED_CHECK_VALUES = [
  ...
  // `customers.source` is the column §11's whole model rests on: it decides
  // whether a row is reachable. A database predating either value would accept
  // rows the current code cannot reason about.
  { table: 'customers', column: 'source', values: ['imported', 'staff'] },
];
...
const REQUIRED_TABLES = ['establishments', 'establishment_social', 'tracked', 'observations', 'connections', 'scans', 'posts', 'post_targets', 'menu_items', 'menu_item_aliases', 'guest_texts', 'customers', 'customer_tags'];
